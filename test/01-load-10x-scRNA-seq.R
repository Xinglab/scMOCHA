...
fn_load_sc_10x <- function(.x, .project = "PBMC10X") {
  # .x is the feature_bc_matrix
  # .x <- feature_path
  
  .counts <- Seurat::Read10X(data.dir = .x)
...
# d <- Seurat::Read10X(
#   data.dir = "data/pbmc_10k_v3_raw_feature_bc_matrix"
# ) %>% 
...
# 
# nrow(xd)
# median(xd$nCount_RNA)
# median(xd$nFeature_RNA)
