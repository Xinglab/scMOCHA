...
importMito.explicit <- function(Afile, Cfile, Gfile, Tfile,
  ...
  importDT <- function(file){
    if(tools::file_ext(file) == "gz"){
      cov <- suppressMessages(data.table::fread(paste0("zcat < ", coverageFile), stringsAsFactors = TRUE))
...
folder <- args[1]
name <- args[2]
folder <- "/home/liuc9/github/scMOCHA/03-ADKP/cromwell-executions/scMOCHA/fa0cc726-77a3-463e-bec5-2c07c4df0f9d/call-call_mt_variants/execution/cluster/final"
